...
-- Everyone can read them. Only system_manager / owner / hr can write to them.
...
CREATE POLICY "System managers can modify sites" ON sites FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'system_manager', 'hr'))
);
...
CREATE POLICY "Broad Read Access to Workers" ON workers FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'ceo', 'hr', 'finance', 'system_manager', 'project_manager', 'supervisor', 'secretary'))
);
...
CREATE POLICY "HR and System can modify workers" ON workers FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'hr', 'system_manager'))
);
...
-- Write: hr, supervisor, secretary, owner
CREATE POLICY "Supervisors and HR can modify attendance" ON attendance FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'hr', 'supervisor', 'secretary'))
);
...
CREATE POLICY "Finance and Global Read Access" ON salary_advances FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'ceo', 'hr', 'finance', 'system_manager'))
);
...
CREATE POLICY "Finance managers can modify advances" ON salary_advances FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'finance', 'hr'))
);
...
CREATE POLICY "Logistics Read Access" ON inventory FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'ceo', 'hr', 'finance', 'system_manager', 'project_manager', 'logistics_manager', 'store_manager'))
);
...
CREATE POLICY "Logistics Write Access" ON inventory FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('owner', 'system_manager', 'project_manager', 'logistics_manager', 'store_manager'))
);
