...
import { DollarSign, Search, Calendar, Calculator, ArrowUpDown, ArrowUp, ArrowDown, Plus, Check, ChevronsUpDown } from 'lucide-react';
...
function AddDeductionForm({ workers, selectedMonth, onSubmit }: { workers: any[]; selectedMonth: string; onSubmit: (data: any) => void }) {
  const [workerId, setWorkerId] = useState('');
  const [amount, setAmount] = useState('');
  const [date, setDate] = useState(format(new Date(), 'yyyy-MM-dd'));
  const [notes, setNotes] = useState('');
