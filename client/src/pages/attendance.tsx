...
export default function AttendancePage() {
  ...
  // Search for workers from other sites by phone or national ID
  const searchCrossSiteWorker = async () => {
    ...
    try {
      ...
      const { data: results, error } = await supabase
        .from('workers')
        .select('*, portfolios(portfolio_name, id), positions(position_name, id), sites(site_name, id)')
        .ilike('name', `%${crossSiteQuery}%`)
        .neq('site_id', userSiteId); // Exclude workers from current supervisor's site

      if (error) throw error;

      const typedResults = (results || []) as any[];
      setCrossSiteResults(typedResults);
      if (results?.length === 0) {
        toast({
          title: "No results",
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
            ...
            {filteredWorkers.length === 0 ? (
              ...
              filteredWorkers.map((worker: any) => {
                ...
                return (
                  <div
                    key={worker.id}
                    className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 rounded-lg border transition-all duration-200 ${
                      alreadyMarked 
