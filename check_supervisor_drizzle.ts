...
async function test() {
  ...
  console.log("Supervisors:", supervisors.map(s => ({ id: s.id, fullName: s.fullName, site_id: s.siteId })));

  if (supervisors.length > 0) {
    for (const sup of supervisors) {
      if (sup.siteId) {
        const site = await db.select().from(sites).where(eq(sites.id, sup.siteId));
        console.log(`Supervisor ${sup.fullName} belongs to site: ${site[0]?.siteName || 'Unknown'}`);
        ...
        console.log(`Supervisor ${sup.fullName} does NOT have a siteId!`);
      }
    }
  }
